...
Was one of the first protocols to use this technology called "blockchain". Satoshi Nakamato released Bitcoin's whitepaper, and it outlined how Bitcoin can be used to make peer-to-peer transactions in a decentralised network. Bitcoin is regarded to as a superior store of value in comparrison to gold. However the limited set of transaction types, data types, and sizes of data storage seemed to limit the sorts of applications that could run directly on Bitcoin. For projects that needed more freedom and flexibility while staying on- chain, a new blockchain was the only option.
